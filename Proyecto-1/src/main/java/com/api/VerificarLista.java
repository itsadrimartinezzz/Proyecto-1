...
public class VerificarLista {
    ...
    public static boolean verificarEstructuraLisp(String expresion) {
        ...
        while (i < tokens.size()) {
            ...
            if (token.equals("(")) {
                ...
                if (operandos < 1) return false;
            }
            // -------------- AHORA '=' ES OPERADOR --------------
            else if (esOperador(token) || esPalabraReservada(token)) {
                // Necesitan al menos un argumento
    ...
    // AÑADIMOS '=' COMO OPERADOR
    private static boolean esOperador(String str) {
        // Incluimos '=' en la lista
        return "+-*/=<".contains(str) || str.equals(">");
        // (O de forma literal: return "+-*/=<".contains(str) || ">".equals(str);)
    }
    ...
    private static boolean esPalabraReservada(String str) {
        // Puedes o no incluir '=' aquí, según prefieras
        // E.g.: return str.matches("DEFUN|SETQ|COND|ATOM|LIST|EQUAL|<|>|T|=");
        // Pero con el approach "operador", no hace falta.
